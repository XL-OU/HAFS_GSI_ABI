module gridio
  ...
  !   2021-10-25  X. Lu and X. Wang - add capability to read/write hydrometeors for
  !               HAFS for ABI assimlation. POC: xuguang.wang@ou.edu
  ! attributes:
  !   language:  f95
  !
  !$$$
...
contains
  subroutine readgriddata(nanal1,nanal2,vars3d,vars2d,n3d,n2d,levels,ndim,ntimes,fileprefixes,filesfcprefixes,reducedgrid,vargrid,qsat)
    ...
    real(r_single), dimension(:,:,:), allocatable ::workvar3d,uworkvar3d,&
                        ...
                        workprsi,qworkvar3d
    real(r_single), dimension(:,:,:), allocatable :: qlworkvar3d,qiworkvar3d, &
                        qsworkvar3d,qrworkvar3d,qgworkvar3d,wworkvar3d
    ...
    integer :: ql_ind,qr_ind,qg_ind,qi_ind,qs_ind,w_ind !Xu
    ...
    qg_ind   = getindex(vars3d, 'qg')   ! QG (3D) !Xu
    ...
    allocate(qlworkvar3d(nx_res,ny_res,nlevs))
    allocate(qrworkvar3d(nx_res,ny_res,nlevs))
    allocate(qsworkvar3d(nx_res,ny_res,nlevs))
    allocate(qiworkvar3d(nx_res,ny_res,nlevs))
    allocate(qgworkvar3d(nx_res,ny_res,nlevs))
    ...
    if (ql_ind > 0) then
        varstrname = 'liq_wat'
        call read_fv3_restart_data3d(varstrname,fv3filename,file_id,qlworkvar3d)
           do k=1,nlevs
              nn = nn_tile0
              do j=1,ny_res
                 do i=1,nx_res
                    nn=nn+1
                    if (qlworkvar3d(i,j,k)<0) qlworkvar3d(i,j,k)=0.
                    vargrid(nn,levels(ql_ind-1)+k,nb,ne)=qlworkvar3d(i,j,k)
                  enddo
               enddo
            enddo
    ...
    if (qr_ind > 0) then
        varstrname = 'rainwat'
        call read_fv3_restart_data3d(varstrname,fv3filename,file_id,qrworkvar3d)
           do k=1,nlevs
              nn = nn_tile0
              do j=1,ny_res
                 do i=1,nx_res
                    nn=nn+1
                    if (qrworkvar3d(i,j,k)<0) qrworkvar3d(i,j,k)=0.
                    vargrid(nn,levels(qr_ind-1)+k,nb,ne)=qrworkvar3d(i,j,k)
                  enddo
               enddo
            enddo
    ...
    if (qi_ind > 0) then
        varstrname = 'ice_wat'
        call read_fv3_restart_data3d(varstrname,fv3filename,file_id,qiworkvar3d)
           do k=1,nlevs
              nn = nn_tile0
              do j=1,ny_res
                 do i=1,nx_res
                    nn=nn+1
                    if (qiworkvar3d(i,j,k)<0) qiworkvar3d(i,j,k)=0.
                    vargrid(nn,levels(qi_ind-1)+k,nb,ne)=qiworkvar3d(i,j,k)
                  enddo
               enddo
            enddo
    ...
    if (qs_ind > 0) then
        varstrname = 'snowwat'
        call read_fv3_restart_data3d(varstrname,fv3filename,file_id,qsworkvar3d)
           do k=1,nlevs
              nn = nn_tile0
              do j=1,ny_res
                 do i=1,nx_res
                    nn=nn+1
                    if (qsworkvar3d(i,j,k)<0) qsworkvar3d(i,j,k)=0.
                    vargrid(nn,levels(qs_ind-1)+k,nb,ne)=qsworkvar3d(i,j,k)
                  enddo
               enddo
            enddo
    ...
    if (qg_ind > 0) then
        varstrname = 'graupel'
        call read_fv3_restart_data3d(varstrname,fv3filename,file_id,qgworkvar3d)
           do k=1,nlevs
              nn = nn_tile0
              do j=1,ny_res
                 do i=1,nx_res
                    nn=nn+1
                    if (qgworkvar3d(i,j,k)<0) qgworkvar3d(i,j,k)=0.
                    vargrid(nn,levels(qg_ind-1)+k,nb,ne)=qgworkvar3d(i,j,k)
                  enddo
               enddo
            enddo
    ...
    if (ps_ind > 0) then
       ...
       if(allocated(qlworkvar3d)) deallocate(qlworkvar3d)
       if(allocated(wworkvar3d)) deallocate(wworkvar3d)
       if(allocated(qrworkvar3d)) deallocate(qrworkvar3d)
       if(allocated(qsworkvar3d)) deallocate(qsworkvar3d)
       if(allocated(qiworkvar3d)) deallocate(qiworkvar3d)
       if(allocated(qgworkvar3d)) deallocate(qgworkvar3d)
    endif
    !======================================================================
    ! Deallocate memory
  ...
  subroutine writegriddata(nanal1,nanal2,vars3d,vars2d,n3d,n2d,levels,ndim,vargrid,no_inflate_flag)
    ...
    integer(i_kind) :: ql_ind,qr_ind,qs_ind,qg_ind,qi_ind !Xu
    ...
    real(r_single), dimension(:,:,:), allocatable ::workvar3d,workinc3d,workinc3d2,uworkvar3d,&
     ...
     real(r_single), dimension(:,:,:), allocatable :: qlworkvar3d,qiworkvar3d,qrworkvar3d, &
                        qsworkvar3d,qgworkvar3d,wworkvar3d
    ...
    if(w_ind>0) then
       ...
       call read_fv3_restart_data3d(varstrname,fv3filename,file_id,wworkvar3d)
         ...
         enddo
         enddo
       wworkvar3d=wworkvar3d+workinc3d

       call write_fv3_restart_data3d(varstrname,fv3filename,file_id,wworkvar3d)
       do k=1,nlevs
          if (nproc .eq. 0)                                               &
             write(6,*) 'WRITEregional : w ',                           &
                 & k, minval(wworkvar3d(:,:,k)), maxval(wworkvar3d(:,:,k))
       enddo
    end if
    ...
    if(ql_ind>0) then
       ...
       call read_fv3_restart_data3d(varstrname,fv3filename,file_id,qlworkvar3d)
         ...
         enddo
         enddo
       qlworkvar3d=qlworkvar3d+workinc3d
       do k=1,nlevs
        do j=1,ny_res
         do i=1,nx_res
          if (qlworkvar3d(i,j,k)<0) qlworkvar3d(i,j,k)=0.0
         end do
        end do
       end do
       call write_fv3_restart_data3d(varstrname,fv3filename,file_id,qlworkvar3d)
       do k=1,nlevs
          if (nproc .eq. 0)                                               &
             write(6,*) 'WRITEregional : ql ',                           &
                 & k, minval(qlworkvar3d(:,:,k)), maxval(qlworkvar3d(:,:,k))
       enddo
    end if
    if(qr_ind>0) then
       ...
       call read_fv3_restart_data3d(varstrname,fv3filename,file_id,qrworkvar3d)
         ...
         enddo
         enddo
       qrworkvar3d=qrworkvar3d+workinc3d

       do k=1,nlevs
        do j=1,ny_res
         do i=1,nx_res
          if (qrworkvar3d(i,j,k)<0) qrworkvar3d(i,j,k)=0.0
         end do
        end do
       end do
       call write_fv3_restart_data3d(varstrname,fv3filename,file_id,qrworkvar3d)
       do k=1,nlevs
          if (nproc .eq. 0)                                               &
             write(6,*) 'WRITEregional : qr ',                           &
                 & k, minval(qrworkvar3d(:,:,k)), maxval(qrworkvar3d(:,:,k))
       enddo
    end if
    if(qs_ind>0) then
       ...
       call read_fv3_restart_data3d(varstrname,fv3filename,file_id,qsworkvar3d)
         ...
         enddo
         enddo
       qsworkvar3d=qsworkvar3d+workinc3d
       do k=1,nlevs
        do j=1,ny_res
         do i=1,nx_res
          if (qsworkvar3d(i,j,k)<0) qsworkvar3d(i,j,k)=0.0
         end do
        end do
       end do

       call write_fv3_restart_data3d(varstrname,fv3filename,file_id,qsworkvar3d)
       do k=1,nlevs
          if (nproc .eq. 0)                                               &
             write(6,*) 'WRITEregional : qs ',                           &
                 & k, minval(qsworkvar3d(:,:,k)), maxval(qsworkvar3d(:,:,k))
       enddo
    end if
    if(qi_ind>0) then
       ...
       call read_fv3_restart_data3d(varstrname,fv3filename,file_id,qiworkvar3d)
         ...
         enddo
         enddo
       qiworkvar3d=qiworkvar3d+workinc3d
       do k=1,nlevs
        do j=1,ny_res
         do i=1,nx_res
          if (qiworkvar3d(i,j,k)<0) qiworkvar3d(i,j,k)=0.0
         end do
        end do
       end do

       call write_fv3_restart_data3d(varstrname,fv3filename,file_id,qiworkvar3d)
       do k=1,nlevs
          if (nproc .eq. 0)                                               &
             write(6,*) 'WRITEregional : qi ',                           &
                 & k, minval(qiworkvar3d(:,:,k)), maxval(qiworkvar3d(:,:,k))
       enddo
    end if
    if(qg_ind>0) then
       ...
       call read_fv3_restart_data3d(varstrname,fv3filename,file_id,qgworkvar3d)
         ...
         enddo
         enddo
       qgworkvar3d=qgworkvar3d+workinc3d
       do k=1,nlevs
        do j=1,ny_res
         do i=1,nx_res
          if (qgworkvar3d(i,j,k)<0) qgworkvar3d(i,j,k)=0.0
         end do
        end do
       end do

       call write_fv3_restart_data3d(varstrname,fv3filename,file_id,qgworkvar3d)
       do k=1,nlevs
          if (nproc .eq. 0)                                               &
             write(6,*) 'WRITEregional : qg ',                           &
                 & k, minval(qgworkvar3d(:,:,k)), maxval(qgworkvar3d(:,:,k))
       enddo
    end if
