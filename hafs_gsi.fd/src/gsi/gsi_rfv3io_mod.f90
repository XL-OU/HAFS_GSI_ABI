...
! set subroutines to public
  ...
  public :: mype_qr,mype_qi,mype_qs,mype_qg,mype_w !Xu
  ...
  integer(i_kind) mype_qr,mype_qi,mype_qs,mype_qg,mype_w !Xu
...
!$$$  end documentation block
    ...
    use radiance_mod, only: n_actual_clouds !Xu
    use gridmod,only : use_fv3_cloud !Xu
...
!   real(r_kind),dimension(:,:,:),pointer::ges_ql=>NULL()
    ...
    real(r_kind),dimension(:,:,:),pointer::ges_w=>NULL() !Xu
    real(r_kind),dimension(:,:,:),pointer::ges_ql=>NULL() !Xu
    real(r_kind),dimension(:,:,:),pointer::ges_qr=>NULL() !Xu
    real(r_kind),dimension(:,:,:),pointer::ges_qi=>NULL() !Xu
    real(r_kind),dimension(:,:,:),pointer::ges_qs=>NULL() !Xu
    real(r_kind),dimension(:,:,:),pointer::ges_qg=>NULL() !Xu
...
!   call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'ql'  ,ges_ql ,istatus );ier=ier+istatus
    ...
    if (n_actual_clouds>0 .or. use_fv3_cloud) then !Xu
     call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'w'  ,ges_w ,istatus);ier=ier+istatus !Xu
     call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'ql'  ,ges_ql ,istatus);ier=ier+istatus !Xu
     call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'qr'  ,ges_qr ,istatus);ier=ier+istatus !Xu
     call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'qi'  ,ges_qi ,istatus);ier=ier+istatus !Xu
     call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'qs'  ,ges_qs ,istatus);ier=ier+istatus !Xu
     call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'qg'  ,ges_qg ,istatus);ier=ier+istatus !Xu
     if( fv3sar_bg_opt == 0) then !Xu
      call gsi_fv3ncdf_read(dynvars,'W','w',ges_w,mype_w,lat2,lon2,.false.) !Xu
      call gsi_fv3ncdf_read(tracers,'LIQ_WAT','liq_wat',ges_ql,mype_ql,lat2,lon2,.false.) !Xu
      call gsi_fv3ncdf_read(tracers,'RAINWAT','rainwat',ges_qr,mype_qr,lat2,lon2,.false.) !Xu
      call gsi_fv3ncdf_read(tracers,'ICE_WAT','ice_wat',ges_qi,mype_qi,lat2,lon2,.false.) !Xu
      call gsi_fv3ncdf_read(tracers,'SNOWWAT','snowwat',ges_qs,mype_qs,lat2,lon2,.false.) !Xu
      call gsi_fv3ncdf_read(tracers,'GRAUPEL','graupel',ges_qg,mype_qg,lat2,lon2,.false.) !Xu
     else !Xu
      call gsi_fv3ncdf_read_v1(dynvars,'W','w',ges_w,mype_w,lat2,lon2,.false.) !Xu
      call gsi_fv3ncdf_read_v1(tracers,'LIQ_WAT','liq_wat',ges_ql,mype_ql,lat2,lon2,.false.) !Xu
      call gsi_fv3ncdf_read_v1(tracers,'RAINWAT','rainwat',ges_qr,mype_qr,lat2,lon2,.false.) !Xu
      call gsi_fv3ncdf_read_v1(tracers,'ICE_WAT','ice_wat',ges_qi,mype_qi,lat2,lon2,.false.) !Xu
      call gsi_fv3ncdf_read_v1(tracers,'SNOWWAT','snowwat',ges_qs,mype_qs,lat2,lon2,.false.) !Xu
      call gsi_fv3ncdf_read_v1(tracers,'GRAUPEL','graupel',ges_qg,mype_qg,lat2,lon2,.false.) !Xu
     end if !Xu
    end if !Xu
    ...
    do k=1,nsig
       do j=1,lon2
          do i=1,lat2
             ...
             if (n_actual_clouds>0 .or. use_fv3_cloud) then !Xu
                if (ges_ql(i,j,k)<0) ges_ql(i,j,k)=0. !Xu
                if (ges_qr(i,j,k)<0) ges_qr(i,j,k)=0. !Xu
                if (ges_qs(i,j,k)<0) ges_qs(i,j,k)=0. !Xu
                if (ges_qi(i,j,k)<0) ges_qi(i,j,k)=0. !Xu
                if (ges_qg(i,j,k)<0) ges_qg(i,j,k)=0. !Xu
             end if
          enddo
       enddo
    enddo
...
!   write out
    if(fv3sar_bg_opt == 0) then
       ...
       if (use_fv3_cloud) then !Xu
         call gsi_fv3ncdf_write_v1(dynvars,'W',ges_w  ,mype_w,add_saved)
