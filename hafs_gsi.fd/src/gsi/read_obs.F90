...
!   2021-10-25  X. Lu and X. Wang - add read_goesabi_netcdf function to read in
...
!  if(lexist .and. trim(dtype) /= 'tcp' )then
  if(lexist .and. trim(dtype) /= 'tcp' .and. trim(dtype) /= 'abi' )then !Xu
      lnbufr = 15
    ...
    do i=1,ndat
       ...
       if (obstype == 't'  .or. obstype == 'uv' .or. &
          ...
          ditype(i) = 'wcp'
               ...
               obstype == 'amsr2'     .or. obstype == 'abi'  .or.  & !Xu &
...
!!!!!!!!!! Xu
               if( trim(infile) == 'g16abi' )then
                call read_goesabi_netcdf(val_dat,ithin,rmesh,platid,gstime,&
