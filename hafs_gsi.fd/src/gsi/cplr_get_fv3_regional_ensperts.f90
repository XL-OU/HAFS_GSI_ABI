...
contains
  ...
  !   2021-10-25  X. Lu and X. Wang - add capability to read/write hydrometeors
  !                     for HAFS ensemble for ABI assimlation. POC: xuguang.wang@ou.edu
  ...
  !$$$ end documentation block
      ...
      real(r_kind),dimension(grd_ens%lat2,grd_ens%lon2,grd_ens%nsig)::w,ql,qr,qi,qs,qg !Xu
...
!            call this%general_read_fv3_regional(fv3_filename,ps,u,v,tv,rh,oz) 
            if ( use_fv3_cloud ) then !Xu
              call this%general_read_fv3_regional(fv3_filename,ps,u,v,tv,rh,oz,w,ql,qr,qi,qs,qg,use_fv3_cloud=use_fv3_cloud)
            else
              call this%general_read_fv3_regional(fv3_filename,ps,u,v,tv,rh,oz,use_fv3_cloud=use_fv3_cloud)
            end if
...
&                      ,g_w,g_ql,g_qr,g_qi,g_qs,g_qg,use_fv3_cloud) !Xu
  !$$$  subprogram documentation block
  ...
  !$$$ end documentation block
      ...
      use gsi_rfv3io_mod,only:mype_w,mype_ql,mype_qr,mype_qs,mype_qi,mype_qg !Xu
      ...
      if ( use_fv3_cloud )then  !Xu
        if(fv3sar_ensemble_opt == 0) then
          ...
          do k=1,grd_ens%nsig
            do j=1,grd_ens%lon2
              do i=1,grd_ens%lat2
                if (g_ql(i,j,k)<0) g_ql(i,j,k)=0. !Xu
                if (g_qr(i,j,k)<0) g_qr(i,j,k)=0. !Xu
                if (g_qs(i,j,k)<0) g_qs(i,j,k)=0. !Xu
                if (g_qi(i,j,k)<0) g_qi(i,j,k)=0. !Xu
                if (g_qg(i,j,k)<0) g_qg(i,j,k)=0. !Xu
              end do
            end do
           end do
        else
