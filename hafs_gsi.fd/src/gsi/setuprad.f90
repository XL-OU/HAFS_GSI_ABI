...
!   2021-10-25  X. Lu and X. Wang - add offline bias correction and cloud impact
!               calculations for ABI assimilation. POC: xuguang.wang@ou.edu
!
!  input argument list:
...
!  real(r_kind) sfc_speed,frac_sea,clw,tpwc,sgagl,tpwc_guess_retrieval
  ...
  real(r_kind) :: cldcod,cldheight,cldf,cldpres,cldprob !Xu
...
!  real(r_kind),dimension(nchanl)::anl_diagtsim_clr,tsim_clr_bc,cldeff_obs,cldeff_sim 
...
!for GMI (dual scan angles)
  ...
  real(r_kind) :: cosza2,Cf,Co,BT_lim,BT_bc !Xu
  integer :: Cc!Xu
  real(r_kind),dimension(60) :: BT_bias=(/-0.331311,-0.591741,-0.740224, &
 & -0.879546,-0.734492,-0.916865,-0.612938,-0.896015,-0.754353,-0.949621, &
 & -0.875048,-1.02419,-1.18139,-1.25717,-1.64042,-1.98019,-2.063,-2.28756, &
 & -2.38545,-2.49814,-2.40649,-1.90498,-1.77206,-0.942054,-0.801652,-0.556863, &
 & -0.672707,-1.2413,-2.20376,-3.1417,-3.71106,-3.78343,-4.1774,-4.31953, &
 & -4.67383,-4.56456,-4.82967,-4.68838,-4.72513,-4.34201,-4.49025,-4.10524, &
 & -3.74719,-3.25128,-2.77628,-2.45711,-1.99328,-1.48931,-0.930325,-0.453402, &
 & -0.285518,0.0881249,0.363749,0.598021,0.495156,0.518311,0.366037,2.21834, &
 & -0.523168,4.45968/)
  real(r_kind),dimension(60) :: BT_ObE=(/2.28108,5.91905,7.03335,8.14013, &
 & 9.19496,10.2875,11.3409,12.4874,13.5602,14.6123,15.6003,16.6937,17.6236, &
 & 18.6528,19.4955,20.2101,21.0805,21.7402,22.2491,22.7657,23.1962,23.1504, &
 & 23.3252,23.2064,23.0013,22.4049,21.5097,19.7568,17.9209,16.4481,15.6147, &
 & 15.0459,14.4838,14.1722,13.4844,13.072,12.3069,11.7885,11.2995,10.9234, &
 & 10.3569,9.88773,9.3234,8.99615,8.50388,7.88611,7.51607,6.90111,6.35742, &
 & 5.83021,5.13298,4.48385,3.94917,3.63285,3.48244,3.48196,3.77775,4.86857, &
 &5.27747,7.0533/)
...
! If both, iextra=2
  ...
  if (abi .or. lwrite_peakwt) then !Xu
     jextra=nchanl_diag+4 !Xu
     iextra=1 !Xu
  end if !Xu
  if (abi .and. lwrite_peakwt) then !Xu
     jextra=nchanl_diag+4 !Xu 
     iextra=2 !Xu
  end if !Xu
...
!       wltm3 = data_s(ism,n)
        if (abi) then !Xu
         cldcod=data_s(nreal-4,n) !Xu
         cldheight=data_s(nreal-3,n) !Xu
         cldpres=data_s(nreal-2,n) !Xu
         cldf=data_s(nreal-1,n) !Xu
         cldprob=data_s(nreal,n) !Xu
        end if !Xu
...
! assignment (needed for vertical localization).
           ...
           if (.not. newpc4pred .and. (.not. abi)) tlapchn(i) = r0_01*tlapchn(i) !Xu
...
!          emissivity sensitivity bias predictor
           ...
           if (abi) then !Xu from Krishnan
            do j=1,npred             !Xu
             pred(j,i)=0.0          !Xu
             predbias(j,i)=0.0      !Xu
            end do                   !Xu
            predbias(npred+1,i)=0.0  !Xu
            predbias(npred+2,i)=0.0  !Xu
           end if !Xu
...
!          Apply SST dependent bias correction with cubic spline
           ...
           if (abi) then !Xu from Krishnan
            ...
            if (i.eq.4) then
             BT_lim=250. !251.
             Cf=max(0.,BT_lim-tsim(i))
             Co=max(0.,BT_lim-tb_obs(i))
             Cc=min(60,nint((Cf+Co)/2.0))
             if ( Cc == 0 ) then
              BT_bc=-0.32391
             else
              if (.not.isnan(tb_obs(i))) BT_bc=BT_bias(Cc)
             end if
            end if
           end if !Xu
...
!          tbcnob = obs - guess before bias correction
           ...
           tbc(i)       = tbcnob(i)                     
           if (abi .and. i.eq.4) tbc(i)=tbc(i)-BT_bc !Xu
...
!       SEVIRI, AHI,ABI Q C
        ...
        else if (seviri .or. ahi ) then !Xu .or. abi) then 
           do i=1,nchanl
              m=ich(i)
              if (varinv(i) < tiny_r_kind) then
                 varinv_use(i) = zero
           ...
           if (.not. abi) call qc_geocsr(nchanl,is,ndat,nsig,ich,sea,land,ice,snow,luse(n), & 
              zsges,trop5,tzbgr,tsavg5,tb_obs_sdv,tbc,tb_obs,tnoise,ptau5,prsltmp,tvp,temp,wmix,emissivity_k,ts,   &
              id_qc,aivals,errf,varinv,varinv_use,cld,cldp,kmax,abi,ahi,seviri) !Xu

           cld = 100-data_s(iclr_sky,n)

!          if rclrsky < 98%, toss data for lowest water-vapor and surface channels
           if(data_s(iclr_sky,n)<98.0_r_kind .and. (.not. abi)) then !Xu
              do i=1,nchanl
...
!          additional qc for surface and  chn7.3: use split window chns to remove opaque clouds
           do i = 1,nchanl
!Xu              if( (abi .or. ahi ).and. i/=2 .and. i/=3 ) then
              if(  ahi .and. i/=2 .and. i/=3 ) then !Xu
...
!       Apply gross check to observations.  Toss obs failing test.
        do i = 1,nchanl
           if (varinv(i) > tiny_r_kind ) then
              ...
              if (abi .and. i.eq.4) then !Xu
               if (abs(tbc(i)) > 99) then !Xu errf(i)) then
                 if(id_qc(i) == igood_qc)id_qc(i)=ifail_gross_qc !Xu
                 varinv(i) = zero !Xu
                 if(luse(n))stats(2,m) = stats(2,m) + one !Xu
                 if(luse(n))aivals(7,is) = aivals(7,is) + one !Xu
               else
                if (Cc==0) then
                 varinv(i)=1.0/(2.05537**2) !Xu
                else
                 if (isnan(tb_obs(i))) then
                  if(id_qc(i) == igood_qc)id_qc(i)=ifail_gross_qc
                  varinv(i)=zero
                  if(luse(n))stats(2,m) = stats(2,m) + one !Xu
                  if(luse(n))aivals(7,is) = aivals(7,is) + one !Xu
                 else
                  varinv(i)=1.0/(BT_ObE(Cc)**2) !Xu
                 end if
                end if
               end if
              else
...
!                toss the observation.
                ...
                if (isnan(tb_obs(i)) .and. abi) then !Xu0 additional
                 if(id_qc(i) == igood_qc)id_qc(i)=ifail_gross_qc
                 varinv(i) = zero
                 if(luse(n))stats(2,m) = stats(2,m) + one
                 if(luse(n))aivals(7,is) = aivals(7,is) + one
...
!           if (abi) then !Xu
!            diagbuf(31) = cldcod !Xu
!            diagbuf(32) = cldheight !Xu
!            diagbuf(33) = cldpres !Xu
!            diagbuf(34) = cldf !Xu
!            diagbuf(35) = cldprob !Xu
!            diagbuf(36) = tsim_clr(i) !Xu
!           end if !Xu
  ...
  if (adp_anglebc) then
    ...
    predbias_angord = zero
              ...
              do i=1,nchanl_diag
                 ...
                 call nc_diag_metadata("TCC",                   sngl(tcc(ich_diag(i)))                           )
                 if (abi) then !Xu
                   call nc_diag_metadata("Cod_EnKF",            sngl(cldcod) ) !Xu
                   call nc_diag_metadata("Cheight_EnKF",            sngl(cldheight) ) !Xu
                   call nc_diag_metadata("Cpres_EnKF",            sngl(cldpres) ) !Xu
                   call nc_diag_metadata("Cfrac_EnKF",            sngl(cldf) ) !Xu
                   call nc_diag_metadata("Cprob_EnKF",            sngl(cldprob) ) !Xu
                   call nc_diag_metadata("TSIM_CLR",            sngl(tsim_clr(ich_diag(i))) ) !Xu
                   call nc_diag_metadata("TSIM",                sngl(tsim(ich_diag(i))) ) !Xu
                 end if !Xu
...
!  if (adp_anglebc) then
!  if (.true.) then
  if(allocated(predbias_angord))  deallocate(predbias_angord) !Xu
!  endif
